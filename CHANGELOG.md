...
- Added Tailwind Play CDN with preflight disabled; semantic CSS stays authoritative. The rejected particle constellation remains removed. The final hybrid backdrop uses a transparent on-demand Three.js shader for broad silk bands plus SVG Bézier ribbons and five crisp Tokyo contour lines as the reliable visible/fallback layer. Rendering occurs only on init/scroll/resize/theme changes (no permanent loop); scroll travel is clamped, fallback remains visible in WebGL mode, and reduced-motion is static.
- Added e2e coverage for theme toggle persistence; manual browser verification covers every exact palette anchor, derived surfaces, glass blur, visible contour geometry, WebGL first-frame marker/alpha coverage, fallback opacity, scroll displacement, reduced-motion freeze, and both themes.
