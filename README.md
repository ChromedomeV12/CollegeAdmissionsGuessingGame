...
| GET | `/api/profiles/:id` | — | One full profile; fetched only after predictions lock or for permanent practice. |
| POST | `/api/scores` | Bearer | `{profileId, score, breakdown}` — keeps the higher score per user/profile and rejects practice-locked profiles. |
| GET | `/api/leaderboard` | — | `[{username, games, avg, best}]` — global rounded average over distinct finalized cases; only players with `>= 5` cases qualify. |
| POST / GET | `/api/locks` | Bearer | Permanently lock a profile to practice mode / list the current user's locks. |
...
| GET / POST / DELETE | `/api/submissions...` | Bearer + `SUBMISSIONS_ENABLED=true` | Maintainer-only consent/ownership workflow. Disabled deployments return `503 {"error":"Submission tools are disabled"}`. |
| GET | `/api/submissions/reddit/callback` | OAuth state + `SUBMISSIONS_ENABLED=true` | Complete temporary Reddit ownership verification. |
...
The first reveal shows only aggregate score, accuracy, and time plus a **5-second Retry case** action. It deliberately hides tier, school, and final-decision details. Retrying makes the second attempt final; letting the timer expire finalizes the first attempt. Either path permanently locks that profile to **Practice**, reveals the full answer, and exposes a **Correct choices** profile tab. Practice runs show full answers immediately, never offer a retry, and never write scores.
...
Runs the Node unit suite (scoring, both no-admit claims, Reddit URL/OAuth/ownership helpers, post sanitization, and design-contrast checks) followed by `e2e_test.cjs`. The browser test registers a throwaway user, verifies Home and theme persistence, finalizes five distinct cases through the single retry window, verifies permanent practice/Correct choices, and checks the seasonless global leaderboard plus rivalry UI and API row. It writes to the real `data/game.db` with unique usernames per run.
...
Player-facing submission UI has been removed. All submission routes except the safe config read are disabled unless the server starts with `SUBMISSIONS_ENABLED=true`; this flag is intended for a maintainer-operated environment, not normal game deployment.

1. An authenticated maintainer submits one Reddit post URL from its author together with the displayed, versioned consent.
...
- **No build pipeline** — in-browser Babel is acceptable for this small deployment; a production-scale frontend would benefit from Vite.
...
- Keep `SUBMISSIONS_ENABLED=false` for the player-facing deployment. If a separate maintainer environment enables it, configure a Reddit web app, exact HTTPS callback URL, and descriptive `REDDIT_USER_AGENT`.
